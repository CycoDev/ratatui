...
public class ParagraphWidgetTests
{
    ...
    public void ParagraphWrapsLongText()
    {
        ...
        var para = Paragraph.Create().WithText("HelloWorld", StyleType.Empty); // width 10
    ...
    public void ParagraphCenterAlignment()
    {
        var backend = new TestBackend();
        var term = new TerminalType(backend, new LoggingContext(null));
        var para = Paragraph.Create().WithText("Hi", StyleType.Empty).WithAlignment(ParagraphAlignment.Center);
    ...
    public void ParagraphRightAlignment()
    {
        var backend = new TestBackend();
        var term = new TerminalType(backend, new LoggingContext(null));
        var para = Paragraph.Create().WithText("Hi", StyleType.Empty).WithAlignment(ParagraphAlignment.Right);
        term.Draw(f => para.Render(f, new Rect(0,0,6,2)));
    ...
    public void ParagraphMaxLinesLimitsRendering()
    {
        var backend = new TestBackend();
        var term = new TerminalType(backend, new LoggingContext(null));
        var para = Paragraph.Create().WithText("LineOneLineTwoLineThree", StyleType.Empty).WithMaxLines(1);
