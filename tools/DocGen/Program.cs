...
internal static class Program
{
    static int Main(string[] args)
    {
        ...
        // Coverage metrics
        int documentedTypes = publicTypes.Count(t => memberDocs.ContainsKey(XmlMemberKeyForType(t)) && !string.IsNullOrWhiteSpace(memberDocs[XmlMemberKeyForType(t)]));
        ...
        foreach (var t in publicTypes)
        {
            ...
            {
                totalMembers++;
                var k = XmlMemberKeyForMember(t, m);
                if (memberDocs.ContainsKey(k) && !string.IsNullOrWhiteSpace(memberDocs[k])) documentedMembers++;
            }
        }
        ...
        double typeCoverage = publicTypes.Count == 0 ? 1 : documentedTypes / (double)publicTypes.Count;
        double memberCoverage = totalMembers == 0 ? 1 : documentedMembers / (double)totalMembers;
